...
public class Password_check   extends BaseTest {
    ...
    public Object[][] testData1() {
        return new Object[][]{{"Ivan", "Pupko", "Obolonskaya 6, 89", "Kiev",
                "420154", "Ukraine", "Kievskaya", "0931545285", "wq1@ukr.net", true, "vanpupko14", "123456789", "not strong Pass", false}};
    }
    @DataProvider(name = "check_Name_field_positive")
    public Object[][] testData_positive() {
        return new Object[][]{{"Bgth&_^43jGJH", true},{"ASDhy658", true}};
    }
    @DataProvider(name = "check_Name_field_negative")
    public Object[][] testData_negative() {
        return new Object[][]{{"h", false, "length<2"},{"Aa!@#$%^&*()-_+=`~/\\,.?><|b /ыйієж", false, "special character"},{ new GenerationField().generateField(100), false, "length100"},{"検査検査検査査検査査検査", false, "japanese"},{"فحصففحصففحصفحصفحصفحص", false, "arabic"} ,{"123456789",false, "123456789"}};
    }
    ...
    public void checkNameField_positive(String value, boolean isPresentExpected) throws InterruptedException {
        registrationPage = EnterValue_toField.enterValue_toField(value, registrationPage.getPass(), registrationPage);
        Thread.sleep(100);
        boolean isPresentActual= isPresentForWait(registrationPage.getInvalidRegisterPassword());
    ...
    public void checkNameField_negative(String value, boolean isPresentExpected, String  sort_of_check) throws InterruptedException {
        ...
        if(isPresentActual) {
            ...
            try {
                ...
                nametext = registrationPage.getPasswordNameTitleText().getAttribute("data-label");
    ...
    // todo Password is not strong enough
    @Test(dataProvider = "newCustomerData_all_fields")
    public void checkPass_strongEnough(String firstName, String lastName, String address, String city,
        ...
        Thread.sleep(100);
        boolean isPresentActual= isPresentForWait(registrationPage.getNotStrongPassRegisterPassword());
        if(isPresentActual) {
            Assert.assertNotEquals(isPresentActual, isPresentExpected);
            String errormessage = driver.findElement(By.xpath("//span[@class='requiredField']")).getText();
            System.out.println("1 errormessage: " +errormessage + " , value: "+ password);
        }else {
            //todo
            String  nametext="";
            String error_message = "some error message";
            try {
                String errormessage = driver.findElement(By.xpath("//span[@class='requiredField']")).getText();
                System.out.println("2 errormessage: " +errormessage+ " , value: "+ password);
                nametext = registrationPage.getPasswordNameTitleText().getAttribute("data-label");
                System.out.println("text "+ nametext);
                String [] color = colorBord_Text(registrationPage, nametext);
                String  border_error_expected="the field border is highlighted in red";
                ...
                error_message = "3 no error message: "+  sort_of_check ;
                Assert.assertEquals(error_message,"some error message" + " , value: " + password);
