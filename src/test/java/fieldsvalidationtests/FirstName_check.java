...
public class FirstName_check extends BaseTest {
    ...
    @DataProvider(name = "check_FirstName_field_positive")
    public Object[][] testData_positive() {
        return new Object[][]{{"Ivan", true},{"abc_12_def", true} ,{"abc 12 def", true}};
    }
    @DataProvider(name = "check_FirstName_field_negative")
    public Object[][] testData_negative() {
        return new Object[][]{{"h", false, "length<2"},{"Aa!@#$%^&*()-_+=`~/\\,.?><|b /ыйієж", false, "special character"},{ new GenerationField().generateField(100), false, "length100"},{"検査", false,"japanese"},{"فحص", false, "arabic"}};
    }
    @Test(dataProvider = "check_FirstName_field_positive"/*, priority = 0*/)
    public void checkFirstNameField_positive(String value, boolean isPresentExpected) throws InterruptedException {
        registrationPage = EnterValue_toField.enterValue_toField(value, registrationPage.getRegisterFirstName(), registrationPage);
            Thread.sleep(100);
             boolean isPresentActual= isPresentForWait(registrationPage.getInvalidRegisterFirstName());
            Assert.assertNotEquals(isPresentActual, isPresentExpected);
    }
    @Test(dataProvider = "check_FirstName_field_negative"/*, priority = 0*/)
    public void checkFirstNameField_negative(String value, boolean isPresentExpected, String  sort_of_check) throws InterruptedException {
        registrationPage = EnterValue_toField.enterValue_toField(value, registrationPage.getRegisterFirstName(), registrationPage);
        Thread.sleep(100);
        boolean isPresentActual= isPresentForWait(registrationPage.getInvalidRegisterFirstName());
        //System.out.println(isPresentActual);
        if(isPresentActual) {
            Assert.assertNotEquals(isPresentActual, isPresentExpected);
            ...
            try {
                ...
                nametext = registrationPage.getFirstNameTitleText().getAttribute("data-label");
